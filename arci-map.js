// ==========================================
// 1. GLOBÁLNY SYSTÉM VRSTIEV
// ==========================================
window.isMapActive = false;

// "Únos" funkcie closeTopModal - aby sme vedeli, že sa niečo zatvorilo
(function() {
    const originalClose = window.closeTopModal;
    window.closeTopModal = function(backHistory = true) {
        if (typeof originalClose === 'function') {
            originalClose(backHistory);
        }
        // Tu nič neotvárame, pretože vrstva s fotkou/mapou už na pozadí BEŽÍ.
        // Iba ak by tvoj kód čistil display:block, poistíme to:
        if (window.isMapActive) {
            document.getElementById('arci-city-game-container').style.display = 'block';
        }
    };
})();

// ==========================================
// 2. LOGIKA MAPY A DETAILU
// ==========================================
...
function startArciCityGame() {
    ...
    // Ak už mapa beží, len ju ukážeme (napr. pri prvom kliknutí z menu)
        ...
        container.innerHTML = `
            ...
            <div id="buildingDetailLayer" onclick="closeBuildingDetail()">
                <div id="detailFrame">
                    ...
                    <div class="detail-hint">👆 KLIKNI NA FOTKU PRE VSTUP</div>
                </div>
            </div>

            <button onclick="exitMap()" class="exit-map-btn">ZAVRIEŤ MAPU</button>
        `;
        renderBuildings();
    }
...
function showBuildingDetail(b) {
    ...
    layer.style.display = 'flex'; // Zobrazíme strednú vrstvu (fotku)
}

function closeBuildingDetail() {
    // Toto zatvorí len fotku a vráti ťa na Mapu, ktorá je pod ňou
    document.getElementById('buildingDetailLayer').style.display = 'none';
}

function executeBuildingAction(event) {
    if (event) event.stopPropagation(); // Aby sa nezatvorila fotka hneď
    
    if (window.currentMapAction) {
        // !!! KĽÚČOVÁ ZMENA !!!
        // Mapu ani fotku NIKDY neschovávame. 
        // Hra sa otvorí nad fotku, pretože v CSS nastavíme tvojim modálom najvyšší z-index.
...
// ... (ostatné funkcie renderBuildings, movePlayer, centerCamera zostávajú rovnaké) ...
...
// Zoznam budov (ak by si ho v súbore nemal)
const buildingsData = {
